...
wgtdat <- data.frame(
  date = ymd(c('2022-02-17', '2022-02-22', '2022-03-04', '2022-03-18', '2022-04-18')),
  weight_lb = c(6, 6, 6, 9, 11),
  weight_oz = c(13, 10, 12.5, 1.6, 10.5)
) %>% 
  mutate(
...
lendat <- data.frame(
    date = ymd(c('2022-02-17', '2022-03-04', '2022-04-18')),
    length_in = c(19.8, 20, 22.7)
  ) %>% 
  mutate(
    months = as.numeric((date - min(date)) / 31),
